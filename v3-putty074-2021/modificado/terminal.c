...
static void term_print_finish(Terminal *term)
{
	...
	/*MEXIDAMINHA
    Coloca o ultimo caractere para eliminar o caractere venus
    */
	MINHA_URL[strlen(MINHA_URL) - 1] = '\0';
	//nMINHA_URL = strlen(MINHA_URL);

	
	/*MEXIDAMINHA
    Executa as opções pré determinadas
    */
	if (strncmp(cFenixReport, MINHA_URL, 6) == 0) {
        meu_AbreFenixReport(term->win, MINHA_URL);
        term->print_job = NULL;
        printer_abort_job(term->print_job);
	}
        ...
        //Só manda para impressora se nao for ações acima
        printer_finish_job(term->print_job);
    }
    //Exemplo de uso do message box
    //meu_MessageBox(term->win, strcat(MINHA_URL,"!!!"), "Titulo");
        ...
        if (term->printing) {
            ...
            if (term->only_printing) {
				if (c == '\033') {
					...
					term->print_state = 0;
					/*MEXIDAMINHA
                    Trata caracteres estranhos, so busca os normais
                    */
					if (!((c == (unsigned char)'\027')||(c == (unsigned char)'\012'))) {
                        if (strlen(MINHA_URL) < MINHA_URL_TAM)
