...
#define PRINTER_DISABLED_STRING "Nenhuma (impressão desabilitada)"
/*mexidaminha*/
#define HOST_BOX_TITLE "Nome do Host (ou endereço IP)"
#define PORT_BOX_TITLE "Porta"
                      ...
                      int protocol, int protcfginfo)
{
    ...
    if (!midsession) {
        ...
        if (!backend_vt_from_proto(PROT_SSH)) {
            ctrl_radiobuttons(s, "Tipo de Conexão", NO_SHORTCUT, 3,
                              HELPCTX(session_hostname),
            ...
            ctrl_radiobuttons(s, "Tipo de Conexão:", NO_SHORTCUT, 4,
                              HELPCTX(session_hostname),
                              config_protocolbuttons_handler, P(hp),
                              "Raw", 'w', I(PROT_RAW),
                              "Telnet", 't', I(PROT_TELNET),
