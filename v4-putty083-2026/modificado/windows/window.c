...
/*MEXIDAMINHA*/
#define IDM_MEU_RELATORIOS 0x0181
#define IDM_MEU_HELP       0x0182
#define IDM_MEU_SUPORTE    0x0183
#define IDM_MEU_TERMSEG    0x0184
#define IDM_MEU_PORTAL     0x0185
/*MEXIDAMINHA*/
...
/*MEXIDAMINHA*/
static void wintw_AbreURL(TermWin *tw, char *cURL) {
    WinGuiSeat *wgs = container_of(tw, WinGuiSeat, termwin);
    if (wgs->term_hwnd)
        ShellExecuteA(wgs->term_hwnd, "open", cURL, NULL, NULL, SW_SHOWDEFAULT);
}
static void wintw_AbreFenixReport(TermWin *tw, char *cURL) {
    WinGuiSeat *wgs = container_of(tw, WinGuiSeat, termwin);
    if (wgs->term_hwnd)
        ShellExecuteA(wgs->term_hwnd, "open", "fenixReport.jar", cURL, "c:\\Fenix\\", SW_SHOWDEFAULT);
}
/*MEXIDAMINHA*/
...
static const TermWinVtable windows_termwin_vt = {
    ...
    /*MEXIDAMINHA*/
    .AbreURL = wintw_AbreURL,
    .AbreFenixReport = wintw_AbreFenixReport,
    /*MEXIDAMINHA*/
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
     ...
     */
    {
        ...
        for (j = 0; j < lenof(wgs->popup_menus); j++) {
            ...
            AppendMenu(m, MF_ENABLED, IDM_COPY, "&Copiar");
            AppendMenu(m, MF_ENABLED, IDM_PASTE, "Co&lar");
        }
        ...
        for (j = 0; j < lenof(wgs->popup_menus); j++) {
            ...
            /* MEXIDAMINHA: KEEP DUPLICATE SESSION AND RECONFIGURE (AND TRANSLATED) */
            AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_DUPSESS, "&Duplicar Sess\xe3" "o");
            AppendMenu(m, MF_ENABLED, IDM_RECONF, "&Configura\xe7\xf5" "es...");
            AppendMenu(m, MF_SEPARATOR, 0, 0);

#if 0 /* MEXIDAMINHA: DISABLE EXTRA SYSTEM MENUS */
...
#endif
            str = dupprintf("&Sobre %s", appname);
            AppendMenu(m, MF_ENABLED, IDM_ABOUT, str);
            sfree(str);
            /*MEXIDAMINHA*/
            /* User requested removal of Relatorios, portal, suporte e ajuda */
            /*MEXIDAMINHA*/
        }
    }
...
static void win_seat_update_specials_menu(Seat *seat)
{
    ...
    for (j = 0; j < lenof(wgs->popup_menus); j++) {
#if 0 /* MEXIDAMINHA: NO SPECIALS MENU */
      ...
      case WM_SYSCOMMAND:
        switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
          ...
          case IDM_MEU_RELATORIOS:
              ...
              break;
          case IDM_MEU_HELP:
              ShellExecuteA(hwnd, "open", "http://localhost:81/help/", NULL, NULL, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_SUPORTE:
              ShellExecuteA(hwnd, "open", "http://www.basepro.com.br/", NULL, NULL, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_TERMSEG:
              ShellExecuteA(hwnd, "open", "http://localhost:82/", NULL, NULL, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_PORTAL:
              ShellExecuteA(hwnd, "open", "http://localhost:81/portal/", NULL, NULL, SW_SHOWDEFAULT);
              break;
          /*MEXIDAMINHA*/
          case IDM_ABOUT:
            showabout(hwnd);
