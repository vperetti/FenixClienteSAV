...
{
    size_t size;
    size = bufchain_size(&term->printer_buf);
    if (size > 5) {
...
static void term_print_finish(Terminal *term)
{
    size_t size;
    char c;
    /*MEXIDAMINHA: URL Trigger intercept*/
    char MINHA_URL[1024];
    memset(MINHA_URL, 0, sizeof(MINHA_URL));
    ...
    if (!term->print_job) {
        ...
        while ((size = bufchain_size(&term->printer_buf)) > 0) {
            ...
            if (c == '\033' || c == '\233') {
                ...
                if (i < 1023) MINHA_URL[i++] = c;
                bufchain_consume(&term->printer_buf, 1);
            }
        }
        if (i > 0 && (MINHA_URL[i-1] == '\r' || MINHA_URL[i-1] == '\n'))
            MINHA_URL[i-1] = '\0';
        if (i > 0 && (MINHA_URL[i-1] == '\r' || MINHA_URL[i-1] == '\n'))
            MINHA_URL[i-1] = '\0';

        if (strncmp("jasper", MINHA_URL, 6) == 0) {
            if (term->win->vt->AbreFenixReport) term->win->vt->AbreFenixReport(term->win, MINHA_URL);
        } else if (strncmp("http", MINHA_URL, 4) == 0) {
            if (term->win->vt->AbreURL) term->win->vt->AbreURL(term->win, MINHA_URL);
        }
    } else {
                    ...
                    switch (ANSI(c, term->esc_query)) {
                      ...
                      case ANSI_QUE('i'): {
                        ...
                        if (term->esc_args[0] == 5 &&
                            ...
                            term->only_printing = !term->esc_query;
                            term->print_state = 0;
                            /* MEXIDAMINHA: term_print_setup bypass */
                            //term_print_setup(term, printer);
