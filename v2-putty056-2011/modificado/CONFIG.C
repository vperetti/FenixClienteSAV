		      ...
		      int midsession, int protocol)
{
    ...
    ssd->okbutton = ctrl_pushbutton(s,
				    (midsession ? "Salvar" : "Abrir"),
				    (char)(midsession ? 'a' : 'o'),
				    HELPCTX(no_help),
				    sessionsaver_handler, P(ssd));
    ssd->okbutton->button.isdefault = TRUE;
    ssd->okbutton->generic.column = 3;
    ssd->cancelbutton = ctrl_pushbutton(s, "Cancelar", 'c', HELPCTX(no_help),
					sessionsaver_handler, P(ssd));
     ...
     */
    /*mexidaminha*/
    str = dupprintf("Opções básicas para sessão %s", appname);
    ctrl_settitle(b, "Sessão", str);
    sfree(str);

    if (!midsession) {
	s = ctrl_getset(b, "Conexão", "hostport",
			"Informe o host name ou endereço IP do servidor");
	ctrl_columns(s, 2, 75, 25);
	c = ctrl_editbox(s, "Nome do Host (ou endereço IP)", 'n', 100,
			 HELPCTX(session_hostname),
			 dlg_stdeditbox_handler, I(offsetof(Config,host)),
			 I(sizeof(((Config *)0)->host)));
	c->generic.column = 0;
	c = ctrl_editbox(s, "Porta", 'p', 100, HELPCTX(session_hostname),
			 dlg_stdeditbox_handler,
			 I(offsetof(Config,port)), I(-1));
	c->generic.column = 1;
	ctrl_columns(s, 1, 100);
	if (backends[3].name == NULL) {
	    ctrl_radiobuttons(s, "Protocolo:", NO_SHORTCUT, 3,
			      HELPCTX(session_hostname),
	    ...
	    ctrl_radiobuttons(s, "Protocolo:", NO_SHORTCUT, 4,
			      HELPCTX(session_hostname),
			      protocolbuttons_handler, P(c),
			      "Raw", 'r', I(PROT_RAW),
			      "Telnet", 't', I(PROT_TELNET),
	...
	s = ctrl_getset(b, "Sessão", "savedsessions",
			"Carregue, salve ou delete uma sessão");
	ctrl_columns(s, 2, 75, 25);
	ssd->sesslist = sesslist;
	ssd->editbox = ctrl_editbox(s, "Sessões Salvas", 'e', 100,
	...
	ssd->loadbutton = ctrl_pushbutton(s, "Carregar", 'l',
					  HELPCTX(session_saved),
					  sessionsaver_handler, P(ssd));
	ssd->loadbutton->generic.column = 1;
	ssd->savebutton = ctrl_pushbutton(s, "Salvar", 'v',
					  HELPCTX(session_saved),
					  sessionsaver_handler, P(ssd));
	ssd->savebutton->generic.column = 1;
	ssd->delbutton = ctrl_pushbutton(s, "Deletar", 'd',
					 HELPCTX(session_saved),
					 sessionsaver_handler, P(ssd));
...
/*MEXIDAMINHA*/
     ...
     * The Terminal panel.
     */
    ctrl_settitle(b, "Terminal", "Opções Gerais do Terminal");
...
/*MEXIDAMINHA
    ...
    ctrl_radiobuttons(s, "Local line editing:", 't', 3,
		      ...
		      "Force off", I(FORCE_OFF), NULL);
MEXIDAMINHA*/
    s = ctrl_getset(b, "Impressão", "printing", "Controle de Impressora");
    ctrl_combobox(s, "Selecione a impressora LOCAL:", 'p', 100,
