...
/*MEXIDAMINHA*/
static bool wintw_ShellExecuteA(TermWin *tw, wchar_t lpOperation, wchar_t lpFile, wchar_t lpParameters,
	wchar_t lpDirectory, int nShowCmd);

//ShellExecute(term->win, "open", MINHA_URL, 0, 0, SW_SHOWDEFAULT);
...
/*MEXIDAMINHA wintw_ShellExecuteA,  */
...
static void start_backend(void)
{
    ...
    if (!vt) {
        char *str = dupprintf("%s Erro interno", appname);
        MessageBox(NULL, "Número de protocolo não suportado encontrado",
    ...
    if (error) {
        char *str = dupprintf("%s Erro", appname);
        char *msg = dupprintf("Não foi possivel abrir conexão com \n%s\n%s",
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
    ...
    if (hr != S_OK && hr != S_FALSE) {
        char *str = dupprintf("%s Erro Fatal", appname);
        MessageBox(NULL, "Falha inicializando subsistema COM",
                   str, MB_OK | MB_ICONEXCLAMATION);
        sfree(str);
        return 1;
    }
     ...
     */
    {
        ...
        AppendMenu(popup_menus[CTXMENU].menu, MF_ENABLED, IDM_COPY, "&Copiar");
        AppendMenu(popup_menus[CTXMENU].menu, MF_ENABLED, IDM_PASTE, "Cola&r");
        ...
        for (j = 0; j < lenof(popup_menus); j++) {
            ...
            str = dupprintf("&Abrir nova janela do %s", appname);
            AppendMenu(m, MF_ENABLED, IDM_DUPSESS, str);
            sfree(str);
            ...
            //           "Sa&ved Sessions");
            AppendMenu(m, MF_ENABLED, IDM_RECONF, "&Opções");
            //AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_COPYALL, "&Copiar toda tela para área de transferência");
            //AppendMenu(m, MF_ENABLED, IDM_CLRSB, "C&lear Scrollback");
            //AppendMenu(m, MF_ENABLED, IDM_RESET, "Rese&t Terminal");
            //AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, (conf_get_int(conf, CONF_resize_action)
                           == RESIZE_DISABLED) ? MF_GRAYED : MF_ENABLED,
                       IDM_FULLSCREEN, "&Tela cheia (alt+enter)");
            //AppendMenu(m, MF_SEPARATOR, 0, 0);
            ...
            //sfree(str);
              ...
              /*MEXIDAMINHA*/
            AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_MEU_RELATORIOS, "R&elatórios");

            //    if (help_path)
            //	AppendMenu(m, MF_ENABLED, IDM_HELP, "&Help");
            str = dupprintf("&Diretórios %s", appname);
            AppendMenu(m, MF_ENABLED, IDM_ABOUT, str);
            sfree(str);
            /*MEXIDAMINHA*/
            AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_MEU_TERMSEG, "Terminal Seguro");
            /*MEXIDAMINHA*/
            AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_MEU_PORTAL, "Portal Intranet");
            /*MEXIDAMINHA*/
            AppendMenu(m, MF_SEPARATOR, 0, 0);
            AppendMenu(m, MF_ENABLED, IDM_MEU_SUPORTE, "Suporte ");
            AppendMenu(m, MF_ENABLED, IDM_MEU_HELP, "Ajuda - Manual do Sistema");
...
char *do_select(SOCKET skt, bool enable)
{
    ...
    if (p_WSAAsyncSelect(skt, hwnd, msg, events) == SOCKET_ERROR) {
        switch (p_WSAGetLastError()) {
          case WSAENETDOWN: /*mexidaminha*/
            return "A conexão caiu";
          default:
            return "WSAAsyncSelect(): erro desconhecido";
        }
    }
    return NULL;
}
...
static void win_seat_connection_fatal(Seat *seat, const char *msg)
{
    char *title = dupprintf("%s Erro Fatal ", appname);
    show_mouseptr(true);
...
void cmdline_error(const char *fmt, ...)
{
    ...
    va_end(ap);
    title = dupprintf("%s Erro de linha de comando", appname);
...
static void win_seat_notify_remote_exit(Seat *seat)
{
    ...
    if (!session_closed &&
        ...
        if (close_on_exit == FORCE_ON ||
            ...
            if (exitcode != INT_MAX) {
                show_mouseptr(true);
                MessageBox(hwnd, "Conexão fechada pelo servidor",
      ...
      case WM_CLOSE:
        {
            char *str;
            show_mouseptr(true);
            str = dupprintf("%s Confirmação de Saida", appname);
      ...
      case WM_SYSCOMMAND:
        switch (wParam & ~0xF) {       /* low 4 bits reserved to Windows */
          ...
          case IDM_RESET:
            ...
            break;
            /*mexidaminha*/
          case IDM_MEU_RELATORIOS:
              ShellExecute(hwnd, "open",
                  "http://localhost:81/relatorios/",
                  0, 0, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_HELP:
              ShellExecute(hwnd, "open",
                  "http://localhost:81/help/",
                  0, 0, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_SUPORTE:
              ShellExecute(hwnd, "open",
                  "http://www.basepro.com.br/",
                  0, 0, SW_SHOWDEFAULT);
              break;
          case IDM_MEU_TERMSEG:
              ShellExecute(hwnd, "open",
                  "http://localhost:82/",
                  0, 0, SW_SHOWDEFAULT);
              break;
...
static bool wintw_ShellExecuteA(TermWin *tw, wchar_t lpOperation, wchar_t lpFile, wchar_t lpParameters,
	...
	if (hwnd)
		//ShellExecute(hwnd, "open", MINHA_URL, 0, 0, SW_SHOWDEFAULT);
		MessageBox(hwnd, lpFile, lpOperation, MB_SYSTEMMODAL | MB_ICONERROR | MB_OK);
		return ShellExecuteA(hwnd, lpOperation, lpFile, lpParameters, lpDirectory, nShowCmd);
		
}

/*MEXIDAMINHA*/
void meu_ShellExecuteA(TermWin *tw, char lpOperation, char lpFile, char *lpParameters,
	char *lpDirectory, int nShowCmd) {

	//if (hwnd)
		//ShellExecute(hwnd, "open", MINHA_URL, 0, 0, SW_SHOWDEFAULT);

		//MessageBox(hwnd, "Mensagem", "Titulo", MB_OK);
		//ShellExecuteA(hwnd, lpOperation, lpFile, NULL, NULL, nShowCmd);
		ShellExecuteA(hwnd, "open", "http://localhost:81/relatorios/", NULL, NULL, SW_SHOWDEFAULT);

}


/*MEXIDAMINHA
Abre url em janela atual*/
...
/*MEXIDAMINHA
Executa o fenixReport.jar com parametros*/
...
void modalfatalbox(const char *fmt, ...)
{
    ...
    va_end(ap);
    show_mouseptr(true);
    title = dupprintf("%s Erro fatal", appname);
...
void nonfatal(const char *fmt, ...)
{
    ...
    va_end(ap);
    show_mouseptr(true);
    title = dupprintf("%s Erro", appname);
    MessageBox(hwnd, message, title, MB_ICONERROR | MB_OK);
    sfree(message);
    sfree(title);
}
